...
const STORAGE_KEY = "scriptsManageRoms.settings";
...
const DEFAULT_CONTENT_COLUMN_WIDTHS = {
    name: 320,
    type: 140,
    size: 120,
    modified: 148,
};
const CONTENT_COLUMN_MIN_WIDTHS = {
    name: 180,
    type: 96,
    size: 92,
    modified: 124,
...
const state = {
    ...
    expandedNotificationIds: new Set(),
    typeSelectBuffer: "",
...
async function bootstrap() {
    await Promise.all([
        ...
        listen("job-finished", async (event) => {
            if (!state.progress || state.progress.jobId !== event.payload.jobId) {
                return;
            }
        ...
        listen(ACTIVATE_LOCATION_EVENT, async (event) => {
    ...
    try {
        state.metadata = await invoke("get_app_metadata");
    }
    catch (error) {
...
function render() {
    if (isConnectionsManagerWindow()) {
        document.title = "Conexiones remotas | scriptsManageRoms";
        appRoot.innerHTML = renderConnectionsManagerWindow();
        bindUi();
        return;
    }
    document.title = "scriptsManageRoms";
    appRoot.innerHTML = `
    <div class="shell">
      <nav class="menu-bar">
        ${renderMenu("Archivo", [
      ...
      <header class="titlebar">
        <div class="titlebox">
          ...
          <div class="titlecopy">
            <div class="apptitle">scriptsManageRoms</div>
        ...
        <div class="searchbox">
          <div class="searchfield">
            ...
            <input id="top-search-input" type="text" placeholder="Buscar en todo el arbol..." value="${escapeAttr(state.topSearchQuery)}" />
            <button id="top-search-clear" class="search-inline-button search-clear-button${state.topSearchQuery.trim() ? " visible" : ""}" ${renderTooltipAttr("Limpiar")}${state.topSearchQuery.trim() ? "" : " disabled"}>${renderIcon("x")}</button>
          </div>
        </div>
      </header>
      ...
      <section class="pathbar">
        ...
        <div class="pathbar-route">
          ...
          <button id="path-go" class="toolbar-button primary"${!state.pathInput.trim() ? " disabled" : ""}>${renderIcon("arrow-right")}Ir</button>
        </div>
      </section>
        ...
        ? `
            <section class="local-search-row">
              <div class="path-input-wrap">
                ...
                <input id="local-search-input" type="search" placeholder="Buscar dentro de la carpeta actual..." value="${escapeAttr(state.localSearchQuery)}" />
              </div>
              <label class="checkbox-line toolbar-check">
                <input id="local-search-recursive" type="checkbox"${state.localSearchRecursive ? " checked" : ""} />
                <span>Recursiva</span>
              </label>
          ...
          <div class="section-head">
            ...
            <div class="section-tools">
              ...
              <div class="view-toggle">
                <button id="view-list" class="toolbar-button compact${state.viewMode === "list" ? " active" : ""}" ${renderTooltipAttr("Vista lista")}>${renderIcon("list")}</button>
                <button id="view-grid" class="toolbar-button compact${state.viewMode === "grid" ? " active" : ""}" ${renderTooltipAttr("Vista rejilla")}>${renderIcon("grid")}</button>
              </div>
...
function isConnectionsManagerWindow() {
    return window.location.hash === CONNECTIONS_WINDOW_HASH;
}
function getConnectionsWindowUrl() {
    const url = new URL(window.location.href);
    url.hash = CONNECTIONS_WINDOW_HASH.slice(1);
    return url.toString();
}
async function openConnectionsManagerWindow() {
    try {
        if (isConnectionsManagerWindow()) {
            return;
        }
...
function reportConnectionsWindowOpenError(error) {
...
function renderMenu(label, items) {
    ...
    <div class="menu-group">
      <button class="menu-trigger${openMenu ? " open" : ""}" data-menu-trigger="${escapeAttr(label)}">${escapeHtml(label)}</button>
      ${openMenu
        ? `
...
function renderConnectionsManagerWindow() {
    return `
...
function renderConnectionsStatusText() {
      ...
      <section class="connections-editor">
        <div class="connection-card-section connection-form-section">
          ...
          <div class="settings-grid connection-grid">
            ...
            <select id="connection-protocol">
              ${["smb", "ssh", "ftp", "ftps"]
        .map((protocol) => `<option value="${protocol}"${draft.protocol === protocol ? " selected" : ""}>${escapeHtml(protocol.toUpperCase())}</option>`)
        .join("")}
            </select>
        ...
        ? `
              <div class="settings-section connection-card-section">
                ...
                <label class="checkbox-line">
                  <input id="connection-ftps-invalid-cert" type="checkbox"${draft.ftpAcceptInvalidCertificates ? " checked" : ""} />
                  <span>Aceptar certificados invalidos</span>
                </label>
                <label class="checkbox-line">
                  <input id="connection-ftps-invalid-host" type="checkbox"${draft.ftpAcceptInvalidHostnames ? " checked" : ""} />
...
function renderTree() {
    ...
    <div class="tree-node${state.selectedTreePath === state.rootPath ? " selected" : ""}">
      <div class="tree-node-row">
        <button class="tree-expander" data-tree-toggle="${escapeAttr(state.rootPath)}"${rootChildren.length === 0 ? " disabled" : ""}>${state.expandedTreePaths.has(state.rootPath) ? "−" : "+"}</button>
        <button class="tree-select" data-tree-select="${escapeAttr(state.rootPath)}" ${renderTooltipAttr(leafName(state.rootPath) || state.rootPath)}>
...
function renderTreeChildren(parentPath) {
    ...
    return children
        ...
        <div class="tree-node${state.selectedTreePath === entry.path ? " selected" : ""}">
          <div class="tree-node-row">
            <button class="tree-expander" data-tree-toggle="${escapeAttr(entry.path)}"${entry.isDir ? "" : " disabled"}>${entry.isDir ? (expanded ? "−" : "+") : ""}</button>
...
function renderFolderTree() {
    ...
    <div class="tree-node${state.selectedTreePath === state.rootPath ? " selected" : ""}">
      <div class="tree-node-row">
        ...
        <button class="tree-select" data-tree-select="${escapeAttr(state.rootPath)}" ${renderTooltipAttr(leafName(state.rootPath) || state.rootPath, { delayMs: 450, overflowSelector: ".tree-label" })}>
          <span class="icon-wrap">${renderIcon("folder")}</span>
          <span class="tree-label" ${renderTooltipAttr(leafName(state.rootPath) || state.rootPath, { delayMs: 450, overflowTarget: "self" })}>${escapeHtml(leafName(state.rootPath) || state.rootPath)}</span>
        </button>
      </div>
...
function renderFolderTreeChildren(parentPath) {
    const children = (state.treeChildren.get(parentPath) ?? []).filter((entry) => entry.isDir);
    return children
        .map((entry) => {
        const expanded = state.expandedTreePaths.has(entry.path);
        ...
        <div class="tree-node${state.selectedTreePath === entry.path ? " selected" : ""}">
          <div class="tree-node-row">
            ...
            overflowSelector: ".tree-label",
              ...
              <span class="icon-wrap">${renderIcon("folder")}</span>
              <span class="tree-label" ${renderTooltipAttr(entry.name, { delayMs: 450, overflowTarget: "self" })}>${escapeHtml(entry.name)}</span>
            </button>
          </div>
...
function renderContentColumnStyle() {
...
function renderContentColumnHeader(key, label) {
...
function parseDirectoryItemCount(entry) {
    if (!entry.isDir) {
        return 0;
    }
...
function applyContentOrdering(entries) {
...
function reapplyContentOrdering(shouldRender = true) {
    state.currentEntries = applyContentOrdering(state.currentEntries);
    if (shouldRender) {
        render();
    }
}
function toggleContentSort(key) {
...
function renderListView() {
    const allSelected = state.currentEntries.length > 0 && state.currentEntries.every((entry) => state.selectedContentPaths.includes(entry.path));
...
function renderListParentEntry(path) {
    return `
    <div class="content-row list-row parent-entry" data-parent-entry="true" data-open-path="${escapeAttr(path)}" data-drop-destination="${escapeAttr(path)}" ${renderTooltipAttr("Directorio superior")}>
      <div class="cell cell-pad-start" aria-hidden="true"></div>
...
function renderListEntry(entry) {
    ...
    return `
    <div class="content-row list-row${selected ? " selected" : ""}" data-content-path="${escapeAttr(entry.path)}" data-content-kind="${entry.isDir ? "dir" : "file"}"${entry.isDir ? ` data-drop-destination="${escapeAttr(entry.path)}"` : ""} ${renderTooltipAttr(entry.name, {
        delayMs: 450,
      ...
      <div class="list-main list-main-surface">
        <div class="cell name-cell">
          <span class="icon-wrap">${renderIcon(entry.isDir ? "folder" : "file")}</span>
          ${renaming
        ? `<input class="inline-rename" data-rename-input="${escapeAttr(entry.path)}" value="${escapeAttr(state.rename?.value ?? entry.name)}" />`
        : `<span class="entry-name" ${renderTooltipAttr(entry.name, { delayMs: 450, overflowTarget: "self" })}>${escapeHtml(entry.name)}</span>`}
        </div>
...
function renderGridParentEntry(path) {
    return `
    <div class="grid-card parent-entry" data-parent-entry="true" data-open-path="${escapeAttr(path)}" data-drop-destination="${escapeAttr(path)}" ${renderTooltipAttr("Directorio superior")}>
...
function renderGridEntry(entry) {
    const selected = state.selectedContentPaths.includes(entry.path);
    const renaming = state.rename?.path === entry.path;
    return `
    <div class="grid-card${selected ? " selected" : ""}" data-content-path="${escapeAttr(entry.path)}" data-content-kind="${entry.isDir ? "dir" : "file"}"${entry.isDir ? ` data-drop-destination="${escapeAttr(entry.path)}"` : ""} ${renderTooltipAttr(entry.name, {
        delayMs: 450,
...
function renderNotificationsIndicator() {
...
function renderNotificationHistoryCard(notification) {
...
function renderContentQuickActions() {
...
function renderContextMenu() {
    ...
    <div class="context-menu" style="left:${menu.x}px; top:${menu.y}px;">
      ${menu.actions
        .map((action) => `
            <button class="context-item${action.danger ? " danger" : ""}" data-context-action="${escapeAttr(action.id)}"${action.disabled ? " disabled" : ""}>
...
function renderExtractionDialog() {
    ...
    <div class="modal-backdrop">
      <div class="modal">
        ...
        <div class="form-grid">
          <label class="checkbox-line"><input id="extract-individual" type="checkbox"${dialog.options.individualFolders ? " checked" : ""} /><span>En carpeta/as individuales de mismo nombre</span></label>
          <label class="checkbox-line"><input id="extract-custom-destination" type="checkbox"${dialog.options.destinationMode === "custom" ? " checked" : ""} /><span>En otra ruta...</span></label>
          <div class="path-picker">
            <input id="extract-destination-path" type="text" value="${escapeAttr(dialog.options.destinationPath ?? "")}" placeholder="Ruta alternativa" ${dialog.options.destinationMode === "custom" ? "" : "disabled"} />
            <button id="extract-pick-path" class="toolbar-button"${dialog.options.destinationMode === "custom" ? "" : " disabled"}>Seleccionar</button>
          </div>
          <label class="checkbox-line"><input id="extract-delete-archives" type="checkbox"${dialog.options.deleteArchives ? " checked" : ""} /><span>Eliminar ficheros comprimidos al terminar</span></label>
          <label class="checkbox-line"><input id="extract-overwrite" type="checkbox"${dialog.options.overwrite ? " checked" : ""} /><span>Permitir sobreescribir</span></label>
        </div>
        <div class="dialog-actions">
...
function renderChdDialog() {
    ...
    <div class="modal-backdrop">
      <div class="modal">
        ...
        .join("<br />")}</div>
        <div class="form-grid">
          <label class="checkbox-line"><input id="chd-delete-originals" type="checkbox"${dialog.options.deleteOriginals ? " checked" : ""} /><span>Eliminar ficheros originales al terminar</span></label>
          <label class="checkbox-line"><input id="chd-name-as-container" type="checkbox"${dialog.options.nameAsContainer ? " checked" : ""} /><span>Nombrar fichero final como carpeta contenedora</span></label>
          <p class="help-line">Util cuando la ROM esta dentro de una subcarpeta y quieres heredar ese nombre.</p>
          ${directoryMode
        ? `
                <label class="checkbox-line"><input id="chd-deposit-parent" type="checkbox"${dialog.options.depositToParent ? " checked" : ""} /><span>Depositar todo en la carpeta padre</span></label>
        ...
        ? `
                <label class="checkbox-line"><input id="chd-delete-subfolders" type="checkbox"${dialog.options.deleteOriginalSubfolders ? " checked" : ""} /><span>Eliminar subcarpetas originales al terminar</span></label>
              `
        : ""}
          <label class="checkbox-line"><input id="chd-overwrite" type="checkbox"${dialog.options.overwrite ? " checked" : ""} /><span>Permitir sobreescribir</span></label>
        </div>
        <div class="dialog-actions">
          <button id="chd-run" class="toolbar-button primary"${dialog.analysis.chdSources.length === 0 ? " disabled" : ""}>Convertir</button>
        </div>
      </div>
    </div>
  `;
}
function renderChdRestoreDialog() {
    ...
    <div class="modal-backdrop">
      <div class="modal">
        ...
        <div class="form-grid">
          <label class="checkbox-line"><input id="restore-individual" type="checkbox"${dialog.options.individualFolders ? " checked" : ""} /><span>En carpeta/as individuales de mismo nombre</span></label>
          <label class="checkbox-line"><input id="restore-custom-destination" type="checkbox"${dialog.options.destinationMode === "custom" ? " checked" : ""} /><span>En otra ruta...</span></label>
          <div class="path-picker">
            <input id="restore-destination-path" type="text" value="${escapeAttr(dialog.options.destinationPath ?? "")}" placeholder="Ruta alternativa" ${dialog.options.destinationMode === "custom" ? "" : "disabled"} />
            <button id="restore-pick-path" class="toolbar-button"${dialog.options.destinationMode === "custom" ? "" : " disabled"}>Seleccionar</button>
          </div>
          <label class="checkbox-line"><input id="restore-delete-chd" type="checkbox"${dialog.options.deleteChd ? " checked" : ""} /><span>Eliminar ficheros .chd al terminar</span></label>
          <label class="checkbox-line"><input id="restore-overwrite" type="checkbox"${dialog.options.overwrite ? " checked" : ""} /><span>Permitir sobreescribir</span></label>
          <label class="checkbox-line"><input id="restore-split-bin" type="checkbox"${dialog.options.splitBin ? " checked" : ""} /><span>Dividir BIN por pistas cuando aplique</span></label>
        </div>
        <div class="dialog-actions">
          <button id="restore-run" class="toolbar-button primary"${dialog.analysis.restorableChds.length === 0 ? " disabled" : ""}>Recuperar</button>
        </div>
      </div>
    </div>
  `;
      ...
      <div class="modal">
        ...
        <div class="settings-section">
          ...
          <div class="settings-grid">
            ...
            <label class="checkbox-line"><input id="settings-compact-ui" type="checkbox"${state.settings.compactUi ? " checked" : ""} /><span>Reducir altura de barras y controles</span></label>
          </div>
        </div>
        ...
        <div class="settings-section">
          ...
          <div class="settings-grid">
            ...
            <div class="settings-stack">
              <label class="checkbox-line"><input id="settings-selection-weight-all" type="checkbox"${state.settings.selectionWeightCalculateAll ? " checked" : ""} /><span>Calcular todo sin limite de subniveles</span></label>
              <div class="settings-inline">
                <span>Subniveles a tener en cuenta</span>
                <input id="settings-selection-weight-depth" type="number" min="1" max="99" step="1" value="${state.settings.selectionWeightMaxDepth}"${state.settings.selectionWeightCalculateAll ? " disabled" : ""} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="settings-section">
          ...
          <div class="settings-grid">
            ...
            <label class="checkbox-line"><input id="settings-confirm-delete" type="checkbox"${state.settings.confirmDelete ? " checked" : ""} /><span>Pedir confirmacion antes de borrar</span></label>
            <label>Popup de borrado</label>
            <label class="checkbox-line"><input id="settings-hide-delete-progress-popup" type="checkbox"${state.settings.hideDeleteProgressPopup ? " checked" : ""} /><span>No mostrar nunca el popup de progreso al eliminar</span></label>
          </div>
        </div>
...
function renderAboutDialog() {
    ...
    <div class="modal-backdrop">
      <div class="modal about-modal">
        ...
        <div class="about-block"><strong>scriptsManageRoms</strong><br />Version app: ${escapeHtml(state.metadata?.appVersion ?? "0.2.0")}</div>
        <div class="about-block"><strong>CHDMAN integrado</strong><br />Version: ${escapeHtml(version)}<br />Autoria / source: ${escapeHtml(author)}<br />Referencia: ${escapeHtml(source)}</div>
      </div>
    </div>
  `;
}
function bindUi() {
    ...
    appRoot.querySelector("#top-search-run")?.addEventListener("click", () => {
        void runGlobalSearch();
    });
    appRoot.querySelector("#top-search-clear")?.addEventListener("click", () => {
        void clearSearchMode();
    });
    appRoot.querySelector("#top-search-input")?.addEventListener("input", (event) => {
        state.topSearchQuery = event.currentTarget.value;
        syncTopSearchUi();
    });
    appRoot.querySelector("#top-search-input")?.addEventListener("keydown", (event) => {
        if (event.key === "Enter") {
            void runGlobalSearch();
        }
    });
...
function bindNotifications() {
    appRoot.querySelector("#notifications-toggle")?.addEventListener("click", (event) => {
        event.stopPropagation();
        state.notificationsOpen = !state.notificationsOpen;
        render();
    });
    appRoot.querySelector(".notifications-dropdown")?.addEventListener("click", (event) => {
        event.stopPropagation();
    });
    appRoot.querySelector("#notifications-clear")?.addEventListener("click", (event) => {
        event.stopPropagation();
        clearNotifications();
    });
    appRoot.querySelectorAll("[data-notification-card]").forEach((card) => {
        card.addEventListener("click", (event) => {
            event.stopPropagation();
            const notificationId = event.currentTarget.dataset.notificationCard;
    ...
    appRoot.querySelectorAll("[data-notification-collapse]").forEach((button) => {
        button.addEventListener("click", (event) => {
            event.preventDefault();
            event.stopPropagation();
            const notificationId = event.currentTarget.dataset.notificationCollapse;
            if (notificationId && state.expandedNotificationIds.has(notificationId)) {
                toggleNotificationExpansion(notificationId);
            }
        });
    });
    appRoot.querySelectorAll("[data-notification-remove]").forEach((button) => {
        button.addEventListener("click", (event) => {
            event.stopPropagation();
            const notificationId = event.currentTarget.dataset.notificationRemove;
            if (notificationId) {
                removeNotification(notificationId);
            }
        });
    });
    appRoot.querySelector("#notification-toast")?.addEventListener("mouseenter", () => {
        pauseActiveNotificationTimer();
    });
    appRoot.querySelector("#notification-toast")?.addEventListener("mouseleave", () => {
        resumeActiveNotificationTimer();
    });
}
function bindMenu() {
    appRoot.querySelectorAll("[data-menu-trigger]").forEach((button) => {
        button.addEventListener("click", (event) => {
            event.stopPropagation();
            const label = event.currentTarget.dataset.menuTrigger ?? null;
            state.menuOpen = state.menuOpen === label ? null : label;
            render();
        });
    });
    appRoot.querySelectorAll("[data-menu-action]").forEach((button) => {
        button.addEventListener("click", (event) => {
            event.stopPropagation();
            state.menuOpen = null;
            void handleMenuAction(event.currentTarget.dataset.menuAction ?? "");
        });
    });
}
function bindPathbar() {
    appRoot.querySelector("#go-up")?.addEventListener("click", () => {
        void navigateUp();
    });
    appRoot.querySelector("#refresh-current")?.addEventListener("click", () => {
        void refreshVisibleData();
    });
    appRoot.querySelector("#path-input")?.addEventListener("input", (event) => {
        state.pathInput = event.currentTarget.value;
    });
    appRoot.querySelector("#path-input")?.addEventListener("keydown", (event) => {
        if (event.key === "Enter") {
            void goToPathInput();
        }
    });
    appRoot.querySelector("#path-go")?.addEventListener("click", () => {
        void goToPathInput();
    });
    appRoot.querySelectorAll("[data-quick-action]").forEach((button) => {
        button.addEventListener("click", () => {
            void handleContextAction(button.dataset.quickAction ?? "");
        });
    });
    appRoot.querySelector("#view-list")?.addEventListener("click", () => {
        state.viewMode = "list";
        render();
    });
    appRoot.querySelector("#view-grid")?.addEventListener("click", () => {
        state.viewMode = "grid";
        render();
    });
    appRoot.querySelector("#zoom-range")?.addEventListener("input", (event) => {
        setContentZoomPreference(Number(event.currentTarget.value));
    });
    bindZoomPreview("#zoom-range");
    appRoot.querySelector("#tree-zoom-range")?.addEventListener("input", (event) => {
        setTreeZoomPreference(Number(event.currentTarget.value));
    });
    bindZoomPreview("#tree-zoom-range");
    appRoot.querySelector("#local-search-input")?.addEventListener("input", (event) => {
        state.localSearchQuery = event.currentTarget.value;
    });
    appRoot.querySelector("#local-search-input")?.addEventListener("keydown", (event) => {
        if (event.key === "Enter") {
            void runLocalSearch();
        }
    });
    appRoot.querySelector("#local-search-recursive")?.addEventListener("change", (event) => {
        state.localSearchRecursive = event.currentTarget.checked;
    });
    appRoot.querySelector("#local-search-run")?.addEventListener("click", () => {
        void runLocalSearch();
    });
    appRoot.querySelector("#local-search-close")?.addEventListener("click", () => {
        state.localSearchOpen = false;
        render();
    });
    appRoot.querySelectorAll("[data-location-root]").forEach((button) => {
        button.addEventListener("click", (event) => {
            const root = event.currentTarget.dataset.locationRoot;
            if (root) {
                void setRootAndNavigate(root);
            }
        });
    });
    appRoot.querySelector("#pick-local-root-inline")?.addEventListener("click", () => {
        void chooseRootDirectory();
    });
    appRoot.querySelector("#add-local-root-inline")?.addEventListener("click", () => {
        void chooseRootDirectory();
    });
    appRoot.querySelector("#open-connections-inline")?.addEventListener("click", () => {
        void openConnectionsManagerWindow();
    });
}
function bindTree() {
    const treeScroll = appRoot.querySelector(".tree-scroll");
    treeScroll?.addEventListener("scroll", (event) => {
        state.treeScrollTop = event.currentTarget.scrollTop;
    });
    appRoot.querySelectorAll("[data-tree-toggle]").forEach((button) => {
        button.addEventListener("pointerdown", (event) => {
            event.preventDefault();
        });
        button.addEventListener("click", (event) => {
            event.stopPropagation();
            const path = event.currentTarget.dataset.treeToggle;
            if (path) {
                void toggleTreePath(path);
            }
        });
    });
    appRoot.querySelectorAll("[data-tree-select]").forEach((button) => {
        button.addEventListener("pointerdown", (event) => {
            event.preventDefault();
        });
        button.addEventListener("click", (event) => {
            event.stopPropagation();
            const path = event.currentTarget.dataset.treeSelect;
            if (path) {
...
function bindContent() {
    appRoot.querySelectorAll("[data-sort-key]").forEach((button) => {
        button.addEventListener("click", (event) => {
            event.preventDefault();
            event.stopPropagation();
    ...
    appRoot.querySelectorAll("[data-resize-key]").forEach((handle) => {
        handle.addEventListener("pointerdown", (event) => {
            ...
            const onPointerMove = (moveEvent) => {
                ...
                state.settings.contentColumnWidths[key] = Math.max(CONTENT_COLUMN_MIN_WIDTHS[key], Math.round(startWidth + (moveEvent.clientX - startX)));
    ...
    appRoot.querySelectorAll("[data-parent-entry]").forEach((row) => {
        row.addEventListener("click", (event) => {
            if (event.target.closest(".cell-pad-start, .cell-check, .cell-pad-end")) {
                return;
            }
    ...
    appRoot.querySelectorAll("[data-content-path]").forEach((row) => {
        ...
        row.addEventListener("click", (event) => {
            ...
            if (event.target.closest("[data-entry-check], .cell-pad-start, .cell-check, .cell-pad-end, .inline-rename")) {
                return;
            }
            void handleContentClick(event, row);
        });
        row.addEventListener("dblclick", (event) => {
            if (event.target.closest(".cell-pad-start, .cell-check, .cell-pad-end, .inline-rename")) {
                return;
            }
    ...
    appRoot.querySelectorAll("[data-entry-check]").forEach((checkbox) => {
        checkbox.addEventListener("click", (event) => {
            event.stopPropagation();
        });
        checkbox.addEventListener("change", (event) => {
            const path = event.currentTarget.dataset.entryCheck;
            if (!path) {
                return;
            }
    ...
    const selectAll = appRoot.querySelector("#content-select-all");
    if (selectAll) {
    ...
    const contentScroll = appRoot.querySelector("#content-scroll");
    ...
    contentScroll?.addEventListener("contextmenu", (event) => {
        if (event.target.closest("[data-content-path]")) {
            return;
        }
        event.preventDefault();
    ...
    appRoot.querySelectorAll("[data-context-action]").forEach((item) => {
        item.addEventListener("click", (event) => {
            event.stopPropagation();
            void handleContextAction(event.currentTarget.dataset.contextAction ?? "");
        });
    });
    appRoot.querySelectorAll("[data-rename-input]").forEach((input) => {
        ...
        input.addEventListener("input", (event) => {
            ...
            state.rename.value = event.currentTarget.value;
        });
...
function bindDialogs() {
    appRoot.querySelector("#progress-close")?.addEventListener("click", () => {
        state.progress = null;
        render();
    });
    appRoot.querySelector("#properties-close")?.addEventListener("click", () => {
        state.propertiesDialog = null;
        render();
    });
    appRoot.querySelector("#extract-close")?.addEventListener("click", () => {
        state.extractionDialog = null;
        render();
    });
    appRoot.querySelector("#extract-individual")?.addEventListener("change", (event) => {
        if (!state.extractionDialog) {
            return;
        }
        state.extractionDialog.options.individualFolders = event.currentTarget.checked;
    });
    appRoot.querySelector("#extract-custom-destination")?.addEventListener("change", (event) => {
        if (!state.extractionDialog) {
            return;
        }
        state.extractionDialog.options.destinationMode = event.currentTarget.checked
            ? "custom"
            : "same";
        render();
    });
    appRoot.querySelector("#extract-destination-path")?.addEventListener("input", (event) => {
        if (state.extractionDialog) {
            state.extractionDialog.options.destinationPath = event.currentTarget.value;
        }
    });
    appRoot.querySelector("#extract-pick-path")?.addEventListener("click", () => {
        void pickExtractionDestination();
    });
    appRoot.querySelector("#extract-delete-archives")?.addEventListener("change", (event) => {
        if (state.extractionDialog) {
            state.extractionDialog.options.deleteArchives = event.currentTarget.checked;
        }
    });
    appRoot.querySelector("#extract-overwrite")?.addEventListener("change", (event) => {
        if (state.extractionDialog) {
            state.extractionDialog.options.overwrite = event.currentTarget.checked;
        }
    });
    appRoot.querySelector("#extract-preview")?.addEventListener("click", () => {
        void loadExtractionPreview();
    });
    appRoot.querySelector("#extract-run")?.addEventListener("click", () => {
        void startExtractionJob();
    });
    appRoot.querySelector("#chd-close")?.addEventListener("click", () => {
        state.chdDialog = null;
        render();
    });
    appRoot.querySelector("#chd-delete-originals")?.addEventListener("change", (event) => {
        if (state.chdDialog) {
            state.chdDialog.options.deleteOriginals = event.currentTarget.checked;
        }
    });
    appRoot.querySelector("#chd-name-as-container")?.addEventListener("change", (event) => {
        if (state.chdDialog) {
            state.chdDialog.options.nameAsContainer = event.currentTarget.checked;
        }
    });
    appRoot.querySelector("#chd-deposit-parent")?.addEventListener("change", (event) => {
        if (!state.chdDialog) {
            return;
        }
        state.chdDialog.options.depositToParent = event.currentTarget.checked;
        render();
    });
    appRoot.querySelector("#chd-delete-subfolders")?.addEventListener("change", (event) => {
        if (state.chdDialog) {
            state.chdDialog.options.deleteOriginalSubfolders = event.currentTarget.checked;
        }
    });
    appRoot.querySelector("#chd-overwrite")?.addEventListener("change", (event) => {
        if (state.chdDialog) {
            state.chdDialog.options.overwrite = event.currentTarget.checked;
        }
    });
    appRoot.querySelector("#chd-run")?.addEventListener("click", () => {
        void startChdJob();
    });
    appRoot.querySelector("#chd-restore-close")?.addEventListener("click", () => {
        state.chdRestoreDialog = null;
        render();
    });
    appRoot.querySelector("#restore-individual")?.addEventListener("change", (event) => {
        if (state.chdRestoreDialog) {
            state.chdRestoreDialog.options.individualFolders = event.currentTarget.checked;
        }
    });
    appRoot.querySelector("#restore-custom-destination")?.addEventListener("change", (event) => {
        if (!state.chdRestoreDialog) {
            return;
        }
        state.chdRestoreDialog.options.destinationMode = event.currentTarget.checked
            ? "custom"
            : "same";
        render();
    });
    appRoot.querySelector("#restore-destination-path")?.addEventListener("input", (event) => {
        if (state.chdRestoreDialog) {
            state.chdRestoreDialog.options.destinationPath = event.currentTarget.value;
        }
    });
    appRoot.querySelector("#restore-pick-path")?.addEventListener("click", () => {
        void pickRestoreDestination();
    });
    appRoot.querySelector("#restore-delete-chd")?.addEventListener("change", (event) => {
        if (state.chdRestoreDialog) {
            state.chdRestoreDialog.options.deleteChd = event.currentTarget.checked;
        }
    });
    appRoot.querySelector("#restore-overwrite")?.addEventListener("change", (event) => {
        if (state.chdRestoreDialog) {
            state.chdRestoreDialog.options.overwrite = event.currentTarget.checked;
        }
    });
    appRoot.querySelector("#restore-split-bin")?.addEventListener("change", (event) => {
        if (state.chdRestoreDialog) {
            state.chdRestoreDialog.options.splitBin = event.currentTarget.checked;
        }
    });
    appRoot.querySelector("#restore-run")?.addEventListener("click", () => {
        void startRestoreJob();
    });
    appRoot.querySelector("#about-close")?.addEventListener("click", () => {
        state.aboutOpen = false;
        render();
    });
    appRoot.querySelector("#settings-close")?.addEventListener("click", () => {
        state.settingsOpen = false;
        render();
    });
    appRoot.querySelector("#settings-font-scale")?.addEventListener("input", (event) => {
        state.settings.fontScale = Number(event.currentTarget.value);
        updateSettingsLabels();
        applyVisualSettings();
    });
    appRoot.querySelector("#settings-default-content-zoom")?.addEventListener("input", (event) => {
        state.settings.defaultContentZoom = Number(event.currentTarget.value);
        updateSettingsLabels();
    });
    appRoot.querySelector("#settings-default-tree-zoom")?.addEventListener("input", (event) => {
        state.settings.defaultTreeZoom = Number(event.currentTarget.value);
        updateSettingsLabels();
    });
    appRoot.querySelector("#settings-selection-weight-all")?.addEventListener("change", (event) => {
        const checked = event.currentTarget.checked;
        state.settings.selectionWeightCalculateAll = checked;
        const depthInput = appRoot.querySelector("#settings-selection-weight-depth");
        if (depthInput) {
            depthInput.disabled = checked;
        }
    });
    appRoot.querySelector("#settings-reset")?.addEventListener("click", () => {
    ...
    appRoot.querySelector("#settings-apply")?.addEventListener("click", () => {
    ...
    appRoot.querySelector("#settings-save")?.addEventListener("click", () => {
        applySettingsFromDialog();
        saveSettings();
        void refreshSelectionSummary([...state.selectedContentPaths]);
        state.settingsOpen = false;
    ...
    appRoot.querySelector("#connections-close")?.addEventListener("click", () => {
        void WebviewWindow.getCurrent().close();
    });
    appRoot.querySelector("#connection-new")?.addEventListener("click", () => {
        startNewConnectionDraft();
    });
    appRoot.querySelectorAll("[data-connection-profile]").forEach((button) => {
        button.addEventListener("click", (event) => {
            const profileId = event.currentTarget.dataset.connectionProfile;
            if (profileId) {
                selectConnectionProfile(profileId);
            }
        });
    });
    appRoot.querySelectorAll("[data-disconnect-session]").forEach((button) => {
        button.addEventListener("click", (event) => {
            event.stopPropagation();
            const sessionId = event.currentTarget.dataset.disconnectSession;
            if (sessionId) {
                void disconnectSession(sessionId);
            }
        });
    });
    appRoot.querySelectorAll("[data-session-root]").forEach((button) => {
        button.addEventListener("click", (event) => {
            event.stopPropagation();
            const rootPath = event.currentTarget.dataset.sessionRoot;
            if (!rootPath) {
                return;
            }
    ...
    appRoot.querySelectorAll("[data-profile-test]").forEach((button) => {
        button.addEventListener("click", (event) => {
            event.stopPropagation();
            const profileId = event.currentTarget.dataset.profileTest;
            if (profileId) {
                void testProfile(profileId);
            }
        });
    });
    appRoot.querySelectorAll("[data-profile-connect]").forEach((button) => {
        button.addEventListener("click", (event) => {
            event.stopPropagation();
            const profileId = event.currentTarget.dataset.profileConnect;
            if (profileId) {
                void connectProfile(profileId);
            }
        });
    });
    appRoot.querySelector("#connection-save")?.addEventListener("click", () => {
        void saveConnectionDraft();
    });
    appRoot.querySelector("#connection-test")?.addEventListener("click", () => {
        void testConnectionDraft();
    });
    appRoot.querySelector("#connection-connect")?.addEventListener("click", () => {
        if (state.editingConnectionProfileId) {
            void connectProfile(state.editingConnectionProfileId);
        }
    });
    appRoot.querySelector("#connection-delete")?.addEventListener("click", () => {
        if (state.editingConnectionProfileId) {
...
function bindSplitter() {
    const splitter = appRoot.querySelector("#splitter");
    if (!splitter) {
        return;
    }
...
function ensureLocalLocation(path) {
...
async function setRootAndNavigate(rootPath, explicitSelection = []) {
...
async function navigateToPath(path) {
    ...
    const entry = await withBusy("Abriendo ruta...", () => invoke("inspect_path", { path: trimmed }));
...
async function loadTreeChildren(path, shouldRender = true) {
    const children = await withBusy("Cargando arbol...", () => invoke("list_children", { path }));
    state.treeChildren.set(path, children);
    if (shouldRender) {
        render();
    }
}
async function loadContent(path, selection = []) {
    const shouldResetScroll = state.currentPath !== path || state.mode !== "browse";
    const entries = await withBusy("Cargando carpeta...", () => invoke("list_children", { path }));
...
async function runGlobalSearch() {
    ...
    const results = await withBusy("Buscando en todo el arbol...", () => invoke("search_entries", { path: state.rootPath, query, recursive: true }));
...
async function runLocalSearch() {
    ...
    const results = await withBusy("Buscando dentro de la carpeta actual...", () => invoke("search_entries", {
...
async function clearSearchMode() {
...
async function handleContentClick(event, row) {
    const path = row.dataset.contentPath;
    if (!path) {
        return;
    }
...
function bindContentMarqueeSelection(contentScroll) {
    if (!contentScroll) {
        return;
    }
    ...
    const updatePreviewSelection = (selection) => {
        const selectedSet = new Set(selection);
        state.selectedContentPaths = state.currentEntries
            .filter((entry) => selectedSet.has(entry.path))
            .map((entry) => entry.path);
        state.anchorPath = state.selectedContentPaths[0] ?? null;
        contentScroll.querySelectorAll("[data-content-path]").forEach((row) => {
            const path = row.dataset.contentPath ?? "";
            row.classList.toggle("selected", selectedSet.has(path));
        });
        contentScroll.querySelectorAll("[data-entry-check]").forEach((checkbox) => {
    ...
    const updateDrag = () => {
        ...
        contentScroll.querySelectorAll("[data-content-path]").forEach((row) => {
            ...
            const intersects = rowRect.right >= selectionRect.left &&
    ...
    contentScroll.addEventListener("pointerdown", (event) => {
        ...
        const gutterStart = (target.closest(".cell-pad-start") || target.closest(".cell-check") || target.closest(".cell-pad-end")) &&
...
async function openPathFromRow(row) {
    const path = row.dataset.contentPath;
...
function handleGlobalKeyDown(event) {
    ...
    const found = state.currentEntries.find((entry) => entry.name.toLowerCase().startsWith(state.typeSelectBuffer.toLowerCase()));
    if (found) {
        setContentSelection([found.path], found.path, true);
    }
}
function selectByContext(path) {
    if (!state.selectedContentPaths.includes(path)) {
        setContentSelection([path], path, true);
    }
}
async function openContextMenu(x, y, requestedPaths, destinationPath, preferRequestedPaths = false) {
    ...
    const analysis = canUseLocalFileTools ? await invoke("scan_selection", { paths }) : null;
    const actions = [
...
async function handleContextAction(action) {
    ...
    switch (action) {
        ...
        case "extract-advanced": {
            ...
            const analysis = await invoke("scan_selection", { paths });
...
async function openSingleSelection(paths = state.selectedContentPaths) {
    if (paths.length !== 1) {
        return;
    }
    try {
...
async function moveDraggedSelection(destination) {
...
async function startTransferJob(paths, destination, operation) {
...
async function determineDeleteBehavior(paths) {
    if (paths.length !== 1) {
        return {
    ...
    const path = paths[0];
    let entry = state.currentEntries.find((item) => item.path === path) ??
        findTreeEntry(path);
    if (!entry) {
        try {
            entry = await invoke("inspect_path", { path });
        }
        catch {
            entry = undefined;
        }
...
async function openPropertiesDialog(paths) {
    if (paths.length === 0) {
        return;
    }
    const requestId = Date.now();
    state.propertiesDialog = await withBusy("Calculando propiedades...", () => invoke("summarize_paths", {
        requestId,
        paths,
        options: {
...
async function createProvisionalEntry(kind) {
    if (!state.currentPath) {
        return null;
    }
    ...
    for (let attempt = 1; attempt <= 500; attempt += 1) {
        ...
        try {
            if (kind === "folder") {
                return await invoke("create_folder", { parent: state.currentPath, name: candidate });
            }
            return await invoke("create_file", { parent: state.currentPath, name: candidate });
        }
        catch {
            continue;
        }
...
async function beginRename(path) {
    if (!path) {
        return;
    }
...
async function finalizeRename(path, rawValue) {
    ...
    try {
        const renamedPath = await invoke("rename_path", { path, newName });
...
async function openChdDialog(paths) {
    if (!paths.every((path) => getEntryLocationKind(path) === "local")) {
...
async function openChdRestoreDialog(paths) {
    if (!paths.every((path) => getEntryLocationKind(path) === "local")) {
...
async function pickExtractionDestination() {
    if (!state.extractionDialog) {
        return;
    }
...
async function loadExtractionPreview() {
    if (!state.extractionDialog) {
        return;
    }
    state.extractionDialog.previewRows = await invoke("build_extraction_preview", {
...
async function refreshVisibleData() {
    ...
    for (const path of expanded) {
        try {
            const children = await invoke("list_children", { path });
            state.treeChildren.set(path, children);
        }
        catch {
            continue;
...
async function loadConnectionProfiles() {
    state.connectionProfiles = await invoke("list_connection_profiles");
    if (state.editingConnectionProfileId) {
...
async function loadActiveConnections() {
    state.activeConnections = await invoke("list_active_connections");
...
function startNewConnectionDraft() {
...
function buildConnectionDraftPayload() {
    return {
...
function bindConnectionDraftInputs() {
    const bindText = (selector, key) => {
        appRoot.querySelector(selector)?.addEventListener("input", (event) => {
            state.connectionDraft[key] = event.currentTarget.value;
        });
    };
    ...
    appRoot.querySelector("#connection-port")?.addEventListener("input", (event) => {
        const value = Number(event.currentTarget.value);
        state.connectionDraft.port = Number.isFinite(value) && value > 0 ? value : null;
    });
    appRoot.querySelector("#connection-protocol")?.addEventListener("change", (event) => {
        state.connectionDraft.protocol = event.currentTarget.value;
        if (!state.connectionDraft.id) {
            state.connectionDraft.port = defaultPortForProtocol(state.connectionDraft.protocol);
        }
        render();
    });
    appRoot.querySelector("#connection-ssh-mode")?.addEventListener("change", (event) => {
        state.connectionDraft.sshMode = event.currentTarget.value;
    });
    appRoot.querySelector("#connection-ftp-mode")?.addEventListener("change", (event) => {
        state.connectionDraft.ftpMode = event.currentTarget.value;
    });
    appRoot.querySelector("#connection-ftps-implicit")?.addEventListener("change", (event) => {
        state.connectionDraft.ftpSecureImplicit = event.currentTarget.checked;
    });
    appRoot.querySelector("#connection-ftps-invalid-cert")?.addEventListener("change", (event) => {
        state.connectionDraft.ftpAcceptInvalidCertificates = event.currentTarget.checked;
    });
    appRoot.querySelector("#connection-ftps-invalid-host")?.addEventListener("change", (event) => {
        state.connectionDraft.ftpAcceptInvalidHostnames = event.currentTarget.checked;
    });
}
async function saveConnectionDraft() {
    try {
        const payload = buildConnectionDraftPayload();
        const saved = await withBusy("Guardando perfil remoto...", () => invoke("save_connection_profile", { profile: payload }));
...
async function deleteConnectionProfileById(profileId) {
    const profile = state.connectionProfiles.find((item) => item.id === profileId);
...
async function connectProfile(profileId, trustCurrentFingerprint = false) {
    try {
        const result = await withBusy("Abriendo conexion remota...", () => invoke("connect_connection_profile", {
            profileId,
            trustCurrentFingerprint,
        }));
        if (result.requiresTrust && result.fingerprint) {
            const accepted = window.confirm(`La huella SSH del host no esta guardada.\n\n${result.fingerprint}\n\nDeseas confiar en ella y continuar?`);
            if (accepted) {
...
async function testConnectionDraft(trustCurrentFingerprint = false) {
    try {
        const result = await withBusy("Probando borrador remoto...", () => invoke("test_connection_profile_payload", {
...
async function testProfile(profileId, trustCurrentFingerprint = false) {
    try {
        const result = await withBusy("Probando conexion remota...", () => invoke("test_connection_profile", {
            profileId,
            trustCurrentFingerprint,
        }));
        if (result.requiresTrust && result.fingerprint) {
...
function applySettingsFromDialog() {
    const theme = appRoot.querySelector("#settings-theme")?.value;
    const defaultView = appRoot.querySelector("#settings-default-view")?.value;
    const fontScale = appRoot.querySelector("#settings-font-scale")?.value;
    const defaultContentZoom = appRoot.querySelector("#settings-default-content-zoom")?.value;
    const defaultTreeZoom = appRoot.querySelector("#settings-default-tree-zoom")?.value;
    const selectionWeightDepth = appRoot.querySelector("#settings-selection-weight-depth")?.value;
    const selectionWeightAll = appRoot.querySelector("#settings-selection-weight-all")?.checked;
    const compactUi = appRoot.querySelector("#settings-compact-ui")?.checked;
    const confirmDelete = appRoot.querySelector("#settings-confirm-delete")?.checked;
    const hideDeleteProgressPopup = appRoot.querySelector("#settings-hide-delete-progress-popup")?.checked;
...
function updateSettingsLabels() {
    const fontValue = appRoot.querySelector("#settings-font-scale-value");
    const contentValue = appRoot.querySelector("#settings-default-content-zoom-value");
    const treeValue = appRoot.querySelector("#settings-default-tree-zoom-value");
    if (fontValue) {
...
function setContentZoomPreference(value, persist = true) {
...
function setTreeZoomPreference(value, persist = true) {
    const normalized = normalizeZoomPercent(value);
...
function applyContentZoom() {
    ...
    appRoot.querySelectorAll("#content-zoom-value").forEach((node) => {
        node.textContent = `${state.zoom}%`;
    });
    const zoomRange = appRoot.querySelector("#zoom-range");
    if (zoomRange) {
        zoomRange.value = String(state.zoom);
    }
}
function applyContentColumnWidths() {
    const body = appRoot.querySelector(".list-content-body");
    if (!body) {
        return;
    }
    const widths = state.settings.contentColumnWidths;
...
function applyTreeZoom() {
    ...
    const treeLabel = appRoot.querySelector("#tree-zoom-value");
    if (treeLabel) {
        treeLabel.textContent = `${state.treeZoom}%`;
    }
    const range = appRoot.querySelector("#tree-zoom-range");
    if (range) {
        range.value = String(state.treeZoom);
    }
}
function syncTopSearchUi() {
    const clearButton = appRoot.querySelector("#top-search-clear");
    if (clearButton) {
...
function bindZoomPreview(selector) {
    const input = appRoot.querySelector(selector);
    const container = input?.closest(".zoom-line");
    if (!input || !container) {
        return;
    }
    const stopDragging = () => {
...
function adjustContextMenuPosition() {
    const menu = appRoot.querySelector(".context-menu");
    if (!menu) {
        return;
    }
    const margin = 8;
...
function setContentSelection(paths, anchor = null, shouldRender = true) {
...
async function refreshSelectionSummary(paths) {
    ...
    try {
        const summary = await invoke("summarize_paths", {
            requestId: token,
            paths,
            options: {
                maxDepth: state.settings.selectionWeightCalculateAll ? null : state.settings.selectionWeightMaxDepth,
...
function beginContentPointerDrag(event, row) {
    const target = event.target;
    if (event.button !== 0) {
        return;
    }
    ...
    const finish = (shouldSuppressClick) => {
...
function isValidDropDestination(destination) {
...
function resolveTreeDropTarget(target) {
    const treeTarget = target?.closest("[data-tree-select]");
...
function resolveContentDropTarget(target) {
    const directTarget = target?.closest("[data-drop-destination]");
    ...
    const fallbackTarget = appRoot.querySelector("#content-scroll");
    if (!fallbackTarget) {
        return null;
    }
    return { element: fallbackTarget, destination: state.currentPath };
}
function bindTreeDropTargets(treeScroll) {
    treeScroll?.addEventListener("dragover", (event) => {
        if (draggingContentPaths.length === 0) {
            return;
        }
        const resolution = resolveTreeDropTarget(event.target);
        if (!resolution) {
...
function bindContentDropTargets(contentScroll) {
    contentScroll?.addEventListener("dragover", (event) => {
        if (draggingContentPaths.length === 0) {
            return;
        }
        const resolution = resolveContentDropTarget(event.target);
        if (!resolution) {
            setActiveDropTarget(null);
            return;
        }
...
function loadSettings() {
    try {
        ...
        parsed.localLocations = Array.isArray(parsed.localLocations)
            ? parsed.localLocations.filter((value) => typeof value === "string" && value.trim().length > 0)
            : [];
        ...
        parsed.contentSortKey = ["name", "type", "size", "modified"].includes(parsed.contentSortKey)
        ...
        parsed.contentColumnWidths = {
            ...
            modified: Math.max(CONTENT_COLUMN_MIN_WIDTHS.modified, Number(parsed.contentColumnWidths.modified) || DEFAULT_CONTENT_COLUMN_WIDTHS.modified),
        };
        return parsed;
    }
    catch {
...
function zoomPercentToScale(value) {
...
function showStatusFlash(text, kind = "info") {
    pushNotification(kind, text);
...
function startActiveNotificationTimer(notificationId, durationMs) {
...
function pauseActiveNotificationTimer() {
...
function resumeActiveNotificationTimer() {
    if (!state.activeNotificationId || !activeNotificationPaused) {
        return;
    }
    activeNotificationStartedAt = Date.now();
...
function getActiveNotificationRemainingMs() {
    if (!state.activeNotificationId) {
        return 0;
    }
...
function toggleNotificationExpansion(notificationId) {
...
function toggleSelection(items, path) {
    return items.includes(path) ? items.filter((item) => item !== path) : [...items, path];
}
function computeRangeSelection(startPath, endPath) {
...
function renderProfileShortLabel(profile) {
...
function getActiveLocationSummary() {
    if (!state.rootPath) {
        return null;
    }
...
function parseRemoteVirtualPath(path) {
    if (!isRemotePath(path)) {
        return null;
    }
...
function resolveInputPath(value) {
    ...
    const remoteBase = (state.currentPath ? parseRemoteVirtualPath(state.currentPath) : null) ??
...
function joinLogicalRemotePath(parentLogicalPath, childName) {
...
function buildPathChain(root, target) {
    const remoteRoot = parseRemoteVirtualPath(root);
...
function renderTooltipAttr(text, options = {}) {
...
function tooltipDelayForTarget(target) {
    const baseDelay = Number(target.dataset.tooltipDelay ?? "0");
    const overflowElement = target.dataset.tooltipOverflow === "self"
        ? target
        : target.dataset.tooltipOverflowSelector
            ? target.querySelector(target.dataset.tooltipOverflowSelector)
            : null;
...
function queueTooltip(target, text, clientX, clientY) {
...
function getActiveRenameInput() {
    return appRoot.querySelector(".inline-rename");
}
function isTargetInsideRenameInput(target, input) {
